...
export default async function ProductPage({params}:{params:Promise<{slug:string}>}){const{slug}=await params;const product=demoProducts.find(x=>x.slug===slug);if(!product)notFound();const type=resolveDetailType(product);return <main><RecentProductTracker product={product}/><SiteHeader/><section className="productDetailWrap"><div className="productDetailTop productDetailMobileStack"><div className={`detailArt productArt ${product.tone}`}><span/></div><div className="productDetailIntro"><span className="productCategory">{product.category}</span><h1>{product.name}</h1><p className="productModel">{product.model}</p><WishlistButton product={product} variant="detail"/><div className="detailPriceBox"><span>{type==="compare"?"현재 비교 최저가":"현재 상품가"}</span>{product.originalPrice!=null&&<del className="originalPrice">{formatWon(product.originalPrice)}</del>}<div className="detailMainPrice">{product.discountRate!=null&&<em>{product.discountRate}%</em>}<strong>{formatWon(product.price)}</strong></div><Badges p={product}/><OfferInfo p={product}/><p>{type==="compare"?`${product.sellers.length}개 판매처의 동일상품을 비교합니다.`:type==="single"?"상품 상세정보와 구매조건을 확인하세요.":"동일상품 매칭 전 유사상품을 함께 보여드립니다."}</p></div></div></div>{type==="compare"?<Compare p={product}/>:type==="single"?<Single p={product}/>:<Similar p={product}/>}<section className="verifiedInfo"><div><span>PRODUCT INFO</span><h2>상품 정보</h2></div><dl><div><dt>상품명</dt><dd>{product.name}</dd></div><div><dt>모델명</dt><dd>{product.model}</dd></div><div><dt>카테고리</dt><dd>{product.category}</dd></div></dl></section>{product.detailImages?.length ? <section className="productDetailImages"><div className="productDetailImagesHead"><span>DETAIL</span><h2>상품 상세</h2></div><div className="productDetailImageStack">{product.detailImages.map((src,index)=><img key={index} src={src} alt={product.name+" 상품 상세 이미지 "+(index+1)} loading="lazy"/>)}</div></section> : null}<Link className="backToSearch" href={`/search?q=${encodeURIComponent(product.name)}`}>비슷한 상품 더 찾아보기 →</Link></section><SiteFooter/><ProductActionBar product={product} type={type}/></main>}