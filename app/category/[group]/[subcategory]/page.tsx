...
export default async function SubcategoryPage({params}:{params:Promise<{group:string;subcategory:string}>}){
 ...
 return <main className="subcategoryPage"><SiteHeader/>
  {validGroup&&<><header className="categoryMenuHeader subcategorySelectedTitle">{validGroup}</header><SubcategoryNav groups={groups} group={validGroup} subcategory={subcategory}/></>}
