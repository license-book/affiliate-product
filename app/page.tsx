...
const categories = ["전체","디지털","가전","생활","주방","선물","PC","모바일"];
...
export default function Home() {
  return <main className="athlerHome">
    ...
    <nav className="mobileTabs" id="categories">{categories.map((x,i)=><Link className={i===0?"active":""} key={x} href={i===0?"/":`/search?q=${encodeURIComponent(x)}`}>{x}</Link>)}</nav>
