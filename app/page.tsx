...
export default function Home(){return <main className="athlerHome"><SiteHeader/><section className="heroViewport" aria-label="추천 기획전"><div className="heroRail">{slides.map((s,i)=><article className={`heroSlide hero-${s[0]}`} key={s[0]}><div className="heroShade"/><div className="heroContent"><span>{s[1]}</span><h1>{s[2].split("|").map(x=><span className="heroLine" key={x}>{x}</span>)}</h1><p>{s[3]}</p><Link href={s[4]}>지금 구경하기 <b>›</b></Link></div><div className="heroDots">{slides.map((_,j)=><i className={j===i?"on":""} key={j}/>)}</div></article>)}</div></section><nav className="categoryIcons" id="categories">{categories.map((x,i)=><Link className={i===0?"active":""} key={x} href={x==="홈"?"/":x==="전체"?"/#best":`/search?q=${encodeURIComponent(x)}`}><span>{categoryIcons[x]}</span><b>{x}</b></Link>)}</nav><div id="recent"><RecentlyViewed/></div><section className="feedSection" id="best"><div className="feedTitle"><div><h2>지금 많이 보는 상품</h2></div><Link href="/search?q=인기상품">더보기 ›</Link></div><ProductFeed limit={8}/></section><footer className="mobileFooter"><strong>9HO</strong><p>상품을 직접 판매하지 않는 가격비교·추천 서비스입니다.<br/>상품 정보와 가격은 판매처에 따라 변경될 수 있습니다.</p><nav><Link href="/about">서비스 안내</Link><Link href="/terms">이용약관</Link><Link href="/privacy">개인정보처리방침</Link></nav><small>일부 링크를 통한 구매 시 9HO가 제휴 수수료를 받을 수 있습니다.</small><em>© 2026 9HO</em></footer></main>}
