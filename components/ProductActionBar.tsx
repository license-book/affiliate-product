...
export default function ProductActionBar({product,type}:{product:DemoProduct;type:"compare"|"single"|"similar"}){
 ...
 return <div className="productActionBar"><div className="productActionIcon"><WishlistButton product={product} variant="detail"/></div><button className="productShareButton" type="button" onClick={share} aria-label="공유"><svg viewBox="0 0 24 24"><circle cx="18" cy="5" r="2.5"/><circle cx="6" cy="12" r="2.5"/><circle cx="18" cy="19" r="2.5"/><path d="m8.2 10.8 7.5-4.4M8.2 13.2l7.5 4.4"/></svg><span>{shared?"복사됨":"공유"}</span></button><button className="productActionPrimary" type="button" onClick={go}><small>{type==="compare"?`최저가 ${new Intl.NumberFormat("ko-KR").format(product.price)}원`:""}</small><b>{label}</b></button></div>
}