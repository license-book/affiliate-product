...
type Slide=readonly [string,string,string,string,string];
...
export default function HeroCarousel({slides}:{slides:readonly Slide[]}){
 ...
 return <section className="heroViewport" aria-label="추천 기획전"><div className="heroRail" ref={rail} onScroll={sync}>{slides.map((s,i)=><article className={`heroSlide hero-${s[0]}`} key={s[0]}><div className="heroShade"/><div className="heroContent"><span>{s[1]}</span><h1>{s[2].split("|").map(x=><span className="heroLine" key={x}>{x}</span>)}</h1><p>{s[3]}</p><Link href={s[4]}>지금 구경하기 <b>›</b></Link></div><div className="heroDots">{slides.map((_,j)=><i className={j===active?"on":""} key={j}/>)}</div></article>)}</div></section>
}