...
export default function LongtailDiscovery(){const[items,setItems]=useState(()=>longtailKeywords.slice(0,COUNT));useEffect(()=>setItems(pick()),[]);return <section className="homeDiscovery homeLongtail"><div className="feedTitle"><div><h2>이런 조건으로 찾고 있나요?</h2><p>구체적인 조건을 바로 검색해보세요</p></div></div><div className="homeLongtailTags">{items.map(x=><Link href={`/pick/${x.slug}`} key={x.label}><svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="10.5" cy="10.5" r="6.5"/><path d="m15.5 15.5 5 5"/></svg><span>{x.label}</span></Link>)}</div></section>}